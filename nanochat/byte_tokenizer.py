...
class ByteTokenizer:
    ...
    def encode_special(self, text):
        ...
        if self.legacy_vocab is not None:
            # BOS is the only special an old base model actually uses (and can
            # emit), so map it into the 256-wide space: 256 -> 0.
            if text == "<|bos|>":
                return tid % self.legacy_vocab
            # The chat specials still get resolved -- Engine.generate_batch asks
            # for them as generation stop-tokens even for a base model. Return
            # their canonical (out-of-range) IDs: a vocab-256 model can never
            # emit an ID >= 256, so these are harmless sentinels that never
            # match a generated token. Actual chat *rendering* (which would feed
            # these IDs into the model) is blocked in render_conversation().
            return tid
        return tid
